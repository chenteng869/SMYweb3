...
<view class="video-card" bindtap="onTap" data-id="{{video.id}}">
  ...
  <view class="video-info">
    ...
    <view
      tt:if="{{video.mountCode}}"
      class="mount-tag"
      catchtap="onMountTap"
      data-mount="{{video.mountCode}}"
    >
      🛍️ {{video.mountName}}
    </view>
  </view>
...
export class BytePayService {
  ...
  async createPrepay(params: {
    ...
    const response = await axios.post(
      'https://payapi.douyinzhifu.com/pay/unifiedorder',
      requestData
    );
...
export const KEYWORD_WHITELIST = ['萨摩亚', '合规', '出海', 'DLC', '海购星', 'KYC'];
...
export class DouyinAiAuditService {
  ...
  async auditText(text: string): Promise<AiAuditResult> {
    const response = await axios.post('https://api.open-douyin.com/audit/text', {
      app_id: this.config.get('DOUYIN_MINIPROGRAM_APPID'),
  ...
  async auditImage(imageUrl: string): Promise<AiAuditResult> {
    const response = await axios.post('https://api.open-douyin.com/audit/image', {
      app_id: this.config.get('DOUYIN_MINIPROGRAM_APPID'),
      access_token: await this.getAccessToken(),
      image_url: imageUrl,
      audit_type: 'image',
  ...
  async auditVideo(videoUrl: string): Promise<AiAuditResult> {
    const response = await axios.post('https://api.open-douyin.com/audit/video', {
      app_id: this.config.get('DOUYIN_MINIPROGRAM_APPID'),
      access_token: await this.getAccessToken(),
      video_url: videoUrl,
      audit_type: 'video',

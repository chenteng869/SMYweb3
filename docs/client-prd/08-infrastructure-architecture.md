...
export class HealthController {
  ...
  @HealthCheck()
  live() {
    return this.health.check([() => this.db.pingCheck('postgres', { timeout: 1500 })]);
  }
...
```typescript
// Session 存储
await this.redis.setex(`sess:${sessionId}`, 30 * 24 * 3600, JSON.stringify(session));
...
export class IngestService {
  async ingestPDF(filePath: string, metadata: DocMetadata) {
    ...
    // 3. Embedding
    const embeddings = await this.embeddingModel.embedDocuments(chunks.map((c) => c.pageContent));

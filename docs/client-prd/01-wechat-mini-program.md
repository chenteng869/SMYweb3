...
export class WxPayService {
  ...
  async verifyCallback(rawBody: string, signature: string, timestamp: string, nonce: string): Promise<WxCallback> {
    // 验签 + 解密
    return this.pay.verify_sign({
      timestamp, nonce, body: rawBody, sign: signature,
    });
  }
}
```
...
App({
  ...
  onLaunch() {
    // 探测系统语言
    const systemInfo = wx.getSystemSystemInfoSync ? wx.getSystemSystemInfoSync() : wx.getSystemInfoSync();

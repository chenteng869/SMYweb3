  ...
  "mcpServers": {
    "postgres-prod-readonly": {
      ...
      "args": [
        ...
        "postgresql://readonly_user:***@prod-db:5432/smy"
      ],
...
```json
{
  "mcpServers": {
    ...
    "lark": {
      "command": "npx",
      "args": ["-y", "@larksuiteoapi/lark-mcp"],
      "env": { "LARK_APP_ID": "${env:LARK_APP_ID}", "LARK_APP_SECRET": "${env:LARK_APP_SECRET}" }
    }
  }
}
```
...
function compact(messages: Message[]): Message[] {
  ...
  return [{ role: 'system', content: `【前 ${old.length} 轮摘要】\n${summary}` }, ...recent];
}
```

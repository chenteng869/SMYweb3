...
export class NotificationsController {
  constructor(private svc: NotificationsService, private audit: AuditService) {}

  @Get() @ApiOperation({ summary: '通知列表' })
  list(@Query() q: any) { return this.svc.list(q); }

  @Post() @ApiOperation({ summary: '创建通知' })
  ...
  @Post('broadcast') @ApiOperation({ summary: '广播通知给所有用户' })
  ...
  @Delete(':id') @ApiOperation({ summary: '删除通知' })

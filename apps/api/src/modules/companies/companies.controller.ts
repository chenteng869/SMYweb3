...
export class CompaniesController {
  constructor(private svc: CompaniesService, private audit: AuditService) {}

  @Get() @ApiOperation({ summary: '公司列表' })
  list(@Query() q: any) { return this.svc.list(q); }

  @Get('stats') @ApiOperation({ summary: '公司统计' })
  stats() { return this.svc.stats(); }

  @Get(':id') @ApiOperation({ summary: '公司详情' })
  detail(@Param('id', ParseIntPipe) id: number) { return this.svc.detail(id); }

  @Post() @ApiOperation({ summary: '创建公司' })
  ...
  @Put(':id') @ApiOperation({ summary: '更新公司' })
  ...
  @Delete(':id') @ApiOperation({ summary: '删除公司' })

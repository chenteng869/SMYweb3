...
export class BanksController {
  constructor(private svc: BanksService, private audit: AuditService) {}

  @Get() @ApiOperation({ summary: '银行账户列表' })
  list(@Query() q: any) { return this.svc.list(q); }

  @Get('stats') @ApiOperation({ summary: '银行账户统计' })
  stats() { return this.svc.stats(); }

  @Get(':id') @ApiOperation({ summary: '银行账户详情' })
  detail(@Param('id', ParseIntPipe) id: number) { return this.svc.detail(id); }

  @Post() @ApiOperation({ summary: '创建银行账户' })
  ...
  @Put(':id') @ApiOperation({ summary: '更新银行账户' })
  ...
  @Delete(':id') @ApiOperation({ summary: '删除银行账户' })

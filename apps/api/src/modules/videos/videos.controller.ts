...
export class VideosController {
  constructor(private svc: VideosService, private audit: AuditService) {}

  @Get() @ApiOperation({ summary: '视频列表' })
  list(@Query() q: any) { return this.svc.list(q); }

  @Get('stats') @ApiOperation({ summary: '视频统计' })
  stats() { return this.svc.stats(); }

  @Get(':id') @ApiOperation({ summary: '视频详情' })
  detail(@Param('id', ParseIntPipe) id: number) { return this.svc.detail(id); }

  @Post() @ApiOperation({ summary: '创建视频' })
  ...
  @Put(':id') @ApiOperation({ summary: '更新视频' })
  ...
  @Delete(':id') @ApiOperation({ summary: '删除视频' })
  ...
  @Get(':id/comments') @ApiOperation({ summary: '视频评论' })
  comments(@Param('id', ParseIntPipe) id: number) { return this.svc.listComments(id); }
}

import { Body, Controller, Delete, Get, Headers, Param, ParseIntPipe, Post, Put, Query, Req } from '@nestjs/common';
...
export class UsersController {
  constructor(private svc: UsersService, private audit: AuditService) {}
  ...
  async updateStatus(@Param('id', ParseIntPipe) id: number, @Body() body: { isActive: boolean; reason?: string }, @Req() req: any, @CurrentUser() user: any) {
    const result = await this.svc.updateStatus(id, body.isActive, body.reason);
    await this.audit.log(req, user.id, body.isActive ? 'enable_user' : 'disable_user', 'users', String(id), body);
    return result;
  }

  @Put(':id/kyc')
  @ApiOperation({ summary: '更新 KYC 状态' })
  async updateKyc(@Param('id', ParseIntPipe) id: number, @Body() body: { kycStatus: string }, @Req() req: any, @CurrentUser() user: any) {
  ...
  async updateLevel(@Param('id', ParseIntPipe) id: number, @Body() body: { dlcLevel: number }, @Req() req: any, @CurrentUser() user: any) {
  ...
  async getRoles(@Query() q: any) { return this.svc.getRoles(q); }

  @Get('roles/:id')
  @ApiOperation({ summary: '获取角色详情' })
  async getRole(@Param('id') id: string) { return this.svc.getRole(Number(id)); }

  @Post('roles')
  @ApiOperation({ summary: '创建角色' })
  async createRole(@Body() dto: any) { return this.svc.createRole(dto); }

  @Put('roles/:id')
  @ApiOperation({ summary: '更新角色' })
  async updateRole(@Param('id') id: string, @Body() dto: any) { return this.svc.updateRole(Number(id), dto); }

  @Delete('roles/:id')
  @ApiOperation({ summary: '删除角色' })
  async deleteRole(@Param('id') id: string) { return this.svc.deleteRole(Number(id)); }
  ...
  async getRiskAssessments(@Query() q: any) { return this.svc.getRiskAssessments(q); }
  ...
  async getRiskStats() { return this.svc.getRiskStats(); }

  // === Audit Logs ===
  @Get('audit/logs')
  @ApiOperation({ summary: '获取操作审计日志' })
  async getAuditLogs(@Query() q: any) { return this.svc.getAuditLogs(q); }

  @Get('audit/stats')
  @ApiOperation({ summary: '审计日志统计' })
  async getAuditLogStats() { return this.svc.getAuditLogStats(); }

  // === Session Management ===
  @Get('sessions')
  @ApiOperation({ summary: '获取活跃会话列表' })
  async getActiveSessions(@Query() q: any) { return this.svc.getAllActiveSessions(q); }
  ...
  async terminateSession(@Param('sessionId') sessionId: string, @Body() body?: { reason?: string }) {
  ...
  async terminateAllSessions(@Param('userId') userId: string, @Headers('x-session-id') exceptId?: string) {
    return this.svc.terminateAllUserSessions(Number(userId), exceptId);
  }

  @Get('sessions/stats')
  @ApiOperation({ summary: '会话统计' })
  async getSessionStats() { return this.svc.getSessionStats(); }

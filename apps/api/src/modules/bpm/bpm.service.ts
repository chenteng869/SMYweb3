...
export class BpmService {
  ...
  async findAllDefs(page: number = 1, pageSize: number = 20, category?: string, status?: string) {
    ...
    const [data, total] = await Promise.all([
      this.prisma.bpmProcessDef.findMany({ where, skip: (page - 1) * pageSize, take: pageSize, orderBy: { updatedAt: 'desc' } }),
  ...
  async deleteDef(id: number) {
    // 检查是否有关联运行中的实例
    const runningCount = await this.prisma.bpmProcessInstance.count({ where: { defId: id, status: { in: ['running', 'suspended'] } } });
  ...
  async publishDef(id: number) {
    ...
    return this.prisma.bpmProcessDef.update({ where: { id }, data: { status: 'active', version: newVersion } });
  }
  ...
  async findAllInstances(page: number = 1, pageSize: number = 20, defId?: number, status?: string, initiator?: string) {
    const where: any = {};
    if (defId) where.defId = defId;
    if (status) where.status = status;
    if (initiator) where.initiator = { contains: initiator };
    const [data, total] = await Promise.all([
      this.prisma.bpmProcessInstance.findMany({ where, skip: (page - 1) * pageSize, take: pageSize, orderBy: { createdAt: 'desc' }, include: { def: true } }),
  ...
  async getInstance(id: number) {
    const instance = await this.prisma.bpmProcessInstance.findUnique({ where: { id }, include: { def: true } });
    if (!instance) return null;
    const tasks = await this.prisma.bpmTask.findMany({ where: { instanceId: id }, orderBy: { createdAt: 'asc' } });
  ...
  async cancelInstance(instanceId: number, reason?: string) {
    const instance = await this.prisma.bpmProcessInstance.findUnique({ where: { id: instanceId } });
    if (!instance || !['running', 'suspended'].includes(instance.status)) throw new Error('实例无法取消');
    return this.prisma.bpmProcessInstance.update({
      where: { id: instanceId },
  ...
  async suspendInstance(instanceId: number) {
    ...
    return this.prisma.bpmProcessInstance.update({ where: { id: instanceId }, data: { status: 'suspended' } });
  }

  async resumeInstance(instanceId: number) {
    const instance = await this.prisma.bpmProcessInstance.findUnique({ where: { id: instanceId } });
    if (!instance || instance.status !== 'suspended') throw new Error('实例未处于暂停状态');
    return this.prisma.bpmProcessInstance.update({ where: { id: instanceId }, data: { status: 'running' } });
  }

  async getInstanceTimeline(instanceId: number) {
    const [tasks, statusChanges] = await Promise.all([
      this.prisma.bpmTask.findMany({ where: { instanceId }, select: { id: true, nodeName: true, status: true, assignee: true, createdAt: true, completedAt: true }, orderBy: { createdAt: 'asc' } }),
      // 假设有状态变更日志表
      [],
    ]);
  ...
  async getInstanceStats(defId?: number) {
    ...
    const [statusCounts, instances] = await Promise.all([
      ...
      this.prisma.bpmProcessInstance.findMany({ where: { ...where, status: 'completed' }, select: { startedAt: true, completedAt: true, durationHours: true } }),
    ]);
    ...
    return { statusDistribution: statusCounts, avgDurationMs: Math.round(avgDuration), totalCompleted: instances.length };
  }

  // ==================== 任务管理 ====================

  async findAllTasks(page: number = 1, pageSize: number = 20, instanceId?: number, assignee?: string, status?: string, type?: string) {
    const where: any = {};
    ...
    const [data, total] = await Promise.all([
      this.prisma.bpmTask.findMany({ where, skip: (page - 1) * pageSize, take: pageSize, orderBy: { createdAt: 'desc' }, include: { instance: { include: { def: true } } } }),
  ...
  async getTask(id: number) {
    return this.prisma.bpmTask.findUnique({ where: { id }, include: { instance: { include: { def: true } } } });
  }
  ...
  async skipTask(id: number, reason?: string) {
    const task = await this.prisma.bpmTask.findUnique({ where: { id } });
    if (!task || task.status !== 'pending') throw new Error('任务不存在或已完成');
    return this.prisma.bpmTask.update({ where: { id }, data: { status: 'skipped', completedAt: new Date() } });
  }
  ...
  async getMyTasks(assignee: string, status?: string) {
    ...
    return this.prisma.bpmTask.findMany({ where, orderBy: { createdAt: 'asc' }, include: { instance: { include: { def: true } } } });
  }
  ...
  async getTaskStats() {
    const [statusCounts, overdueCount, assigneeLoad] = await Promise.all([
      ...
      this.prisma.bpmTask.groupBy({ by: ['assignee'], where: { status: 'pending' }, _count: { id: true } }),
    ]);
    return { statusDistribution: statusCounts, overdueCount, assigneeLoad: assigneeLoad.sort((a, b) => b._count.id - a._count.id).slice(0, 10) };
  }

  // ==================== 流程监控 ====================

  async findAllMetrics(page: number = 1, pageSize: number = 20, defId?: number, dateFrom?: Date, dateTo?: Date) {
    const where: any = {};
    if (defId) where.defId = defId;
    ...
    const [data, total] = await Promise.all([
      this.prisma.bpmMonitorMetric.findMany({ where, skip: (page - 1) * pageSize, take: pageSize, orderBy: { date: 'desc' } }),
  ...
  async getDashboardData() {
    ...
    const [runningCount, todayCompleted, avgDurationData, bottlenecks, recentEfficiency] = await Promise.all([
      this.prisma.bpmProcessInstance.count({ where: { status: 'running' } }),
      this.prisma.bpmProcessInstance.count({ where: { status: 'completed', completedAt: { gte: todayStart } } }),
      this.prisma.bpmProcessInstance.aggregate({ where: { status: 'completed', completedAt: { gte: new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000) } }, _avg: { durationHours: true } }),
      this.getBottlenecks(),
      this.prisma.bpmMonitorMetric.findMany({ where: { date: { gte: new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000) } }, orderBy: { date: 'asc' }, take: 30 }),
    ]);
    ...
    return {
      ...
      bottlenecks,
      efficiencyTrend: recentEfficiency.map(m => ({ date: m.date, completionRate: m.completedCnt, avgDuration: m.avgDurationHrs })),
    };
  }
  ...
  async getEfficiencyReport(category?: string) {
    ...
    const defs = await this.prisma.bpmProcessDef.findMany({ where: category ? { category } : {}, select: { id: true, name: true, category: true } });
    ...
    for (const def of defs) {
      const [total, completed, avgDuration] = await Promise.all([
        ...
        this.prisma.bpmProcessInstance.aggregate({ where: { defId: def.id, status: 'completed' }, _avg: { durationHours: true } }),
      ]);
  ...
  async getSlaCompliance(slaHours: number = 24) {
    ...
    const [total, withinSla, breached] = await Promise.all([
      ...
      this.prisma.bpmProcessInstance.count({ where: { status: 'completed', durationHours: { lte: slaHrs } } }),
      this.prisma.bpmProcessInstance.count({ where: { status: 'completed', durationHours: { gt: slaHrs } } }),
    ]);

...
export class MediaController {
  constructor(private svc: MediaService, private audit: AuditService) {}

  @Get() @ApiOperation({ summary: '媒体发文列表' })
  list(@Query() q: any) { return this.svc.list(q); }

  @Get('stats') @ApiOperation({ summary: '媒体统计' })
  stats() { return this.svc.stats(); }

  @Get(':id') @ApiOperation({ summary: '媒体详情' })
  detail(@Param('id', ParseIntPipe) id: number) { return this.svc.detail(id); }

  @Post() @ApiOperation({ summary: '创建媒体发文' })
  ...
  @Put(':id') @ApiOperation({ summary: '更新媒体发文' })
  ...
  @Delete(':id') @ApiOperation({ summary: '删除媒体发文' })

...
export class PaymentsController {
  constructor(private svc: PaymentsService, private audit: AuditService) {}

  // Channels
  @Get('channels') @ApiOperation({ summary: '支付渠道列表' })
  channels() { return this.svc.channels(); }

  @Get('channels/:id') @ApiOperation({ summary: '支付渠道详情' })
  channelDetail(@Param('id', ParseIntPipe) id: number) { return this.svc.channelDetail(id); }

  @Post('channels') @ApiOperation({ summary: '创建支付渠道' })
  ...
  @Put('channels/:id') @ApiOperation({ summary: '更新支付渠道' })
  async updateChannel(@Param('id', ParseIntPipe) id: number, @Body() body: any, @Req() req: any, @CurrentUser() user: any) {
    const r = await this.svc.updateChannel(id, body);
    await this.audit.log(req, user.id, 'update_channel', 'payments', String(id), body);
    return r;
  }

  @Delete('channels/:id') @ApiOperation({ summary: '删除支付渠道' })
  async deleteChannel(@Param('id', ParseIntPipe) id: number, @Req() req: any, @CurrentUser() user: any) {
  ...
  // Transactions
  @Get('transactions') @ApiOperation({ summary: '交易流水列表' })
  listTxns(@Query() q: any) { return this.svc.listTransactions(q); }

  @Post('transactions') @ApiOperation({ summary: '创建交易' })
  ...
  @Put('transactions/:id') @ApiOperation({ summary: '更新交易状态' })
  async updateTxn(@Param('id', ParseIntPipe) id: number, @Body() body: any, @Req() req: any, @CurrentUser() user: any) {
  ...
  // Exchange Rates
  @Get('rates') @ApiOperation({ summary: '汇率列表' })
  rates() { return this.svc.rates(); }

  @Post('rates') @ApiOperation({ summary: '新增/更新汇率' })
  ...
  @Put('rates/:id') @ApiOperation({ summary: '更新汇率' })
  async updateRate(@Param('id', ParseIntPipe) id: number, @Body() body: { rate: number }, @Req() req: any, @CurrentUser() user: any) {
    const r = await this.svc.updateRate(id, body.rate);
    await this.audit.log(req, user.id, 'update_rate', 'payments', String(id), body);
    return r;
  }

  @Get('stats') @ApiOperation({ summary: '支付统计' })

...
export class DlcController {
  constructor(private svc: DlcService, private audit: AuditService) {}

  @Get('levels') @ApiOperation({ summary: 'DLC 等级列表' })
  levels() { return this.svc.listLevels(); }

  @Post('levels') @ApiOperation({ summary: '新增 DLC 等级' })
  ...
  @Put('levels/:id') @ApiOperation({ summary: '更新 DLC 等级' })
  async updateLevel(@Param('id', ParseIntPipe) id: number, @Body() body: any, @Req() req: any, @CurrentUser() user: any) {
    const r = await this.svc.updateLevel(id, body);
    await this.audit.log(req, user.id, 'update_dlc_level', 'dlc', String(id), body);
    return r;
  }

  @Get('transactions') @ApiOperation({ summary: 'DVC 流水' })
  transactions(@Query() q: any) { return this.svc.listTransactions(q); }

  @Post('transactions') @ApiOperation({ summary: '手动发放 DVC' })
  ...
  @Get('stats') @ApiOperation({ summary: 'DLC 统计' })

...
export class TiktokGlobalAdapter extends BasePlatformAdapter {
  ...
  normalizeInfluencer(raw: Record<string, unknown>): NormalizedInfluencer {
    ...
    return {
      ...
      platformId: oid,
      displayName: String(ui.display_name || ui.nickname || 'TikTok User'),
      username: String(ui.unique_id || ''),
      bio: String(ui.bio_description || ''),
      followerCount: Number(ui.follower_count ?? 0),
      followingCount: Number(ui.following_count ?? 0),
      postCount: Number(ui.video_count ?? 0),
      engagementRate: this.calcEngagement(ui),
      avatarUrl: String(ui.avatar_url || ui.avatar_depth_url || ''),
      profileUrl:
        String(ui.profile_deep_link || '') ||
        (ui.unique_id ? `https://www.tiktok.com/@${ui.unique_id}` : undefined),
      isVerified: Boolean(ui.is_verified ?? false),

...
export class DvsfController {
  constructor(
    private svc: DvsfService,
    private audit: AuditService
  ) {}

  @Get('pools')
  @ApiOperation({ summary: '分红池列表' })
  pools() {
    return this.svc.listPools();
  }

  @Get('pools/:id')
  @ApiOperation({ summary: '分红池详情' })
  poolDetail(@Param('id', ParseIntPipe) id: number) {
    return this.svc.poolDetail(id);
  }

  @Post('pools')
  @ApiOperation({ summary: '创建分红池' })

...
export class OrdersController {
  constructor(private svc: OrdersService, private audit: AuditService) {}

  @Get() @ApiOperation({ summary: '工单列表' })
  list(@Query() q: any) { return this.svc.list(q); }

  @Get('stats') @ApiOperation({ summary: '工单统计' })
  stats() { return this.svc.stats(); }

  @Get(':id') @ApiOperation({ summary: '工单详情' })
  detail(@Param('id', ParseIntPipe) id: number) { return this.svc.detail(id); }

  @Post() @ApiOperation({ summary: '创建工单' })
  ...
  @Put(':id') @ApiOperation({ summary: '更新工单' })
  ...
  @Put(':id/status') @ApiOperation({ summary: '更新工单状态' })
  async updateStatus(@Param('id', ParseIntPipe) id: number, @Body() body: { status: string; progress?: number; notes?: string }, @Req() req: any, @CurrentUser() user: any) {
  ...
  @Delete(':id') @ApiOperation({ summary: '删除工单' })

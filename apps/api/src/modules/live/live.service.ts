...
export class LiveService {
  ...
  async findAllPlatforms(page: number = 1, pageSize: number = 20, type?: string, status?: string) {
    ...
    const [data, total] = await Promise.all([
      this.prisma.livePlatform.findMany({
        where,
  ...
  async getPlatform(id: number) {
    ...
    const [roomsCount, streamsCount, analytics] = await Promise.all([
      ...
      this.prisma.liveAnalytics.findMany({
        where: { platformId: id },
        orderBy: { date: 'desc' },
        take: 7,
      }),
    ...
    return {
      ...platform,
      _count: { rooms: roomsCount, streams: streamsCount },
  ...
  async findAllRooms(
    ...
    const enriched = await Promise.all(
      data.map(async (r) => {
        ...
        return {
          ...r,
          _count: { streams: streamsCount, comments: commentsCount, schedules: schedulesCount },
        };
      })
    );
    return { data: enriched, total, page, pageSize };
  ...
  async getRoom(id: number) {
    ...
    if (!room) return null;
    const commentsCount = await this.prisma.liveComment.count({
  ...
  async endLive(roomId: number) {
    const room = await this.prisma.liveRoom.findUnique({
      where: { id: roomId },
      include: {
        streams: { where: { status: 'living' }, orderBy: { startedAt: 'desc' }, take: 1 },
      },
    });
  ...
  async getRoomStats() {
    const [totalRooms, byStatus, byPlatform, todayLiving] = await Promise.all([
      ...
      this.prisma.liveStream.count({
        where: { status: 'living', startedAt: { gte: new Date(new Date().setHours(0, 0, 0, 0)) } },
      }),
    ]);
    // 获取总观看数
  ...
  async getStreamStats(platformId?: number, days: number = 7) {
    ...
    if (platformId) where.platformId = platformId;
    const [totalSessions, streams, viewsAgg, likesAgg, giftsAgg, followersAgg, durationAgg] =
      await Promise.all([
        this.prisma.liveStream.count({ where }),
        this.prisma.liveStream.findMany({
          where,
    ...
    for (const s of streams) {
      ...
      if (!trendMap[day])
        trendMap[day] = { sessions: 0, views: 0, likes: 0, followers: 0, durationMin: 0 };
  ...
  async getDashboardData() {
    ...
    const platformData = await Promise.all(
      platforms.map(async (p) => {
        ...
        const living = await this.prisma.liveRoom.count({
          where: { platformId: p.id, status: 'living' },
        });
        ...
        return {
          ...
          rooms,
          living,
          todayViews: todayViews._sum.totalViews || 0,
          todayLikes: todayViews._sum.totalLikes || 0,
        };
      })
    );
    return {
  ...
  async getPlatformComparison(days: number = 30) {
    const since = new Date();
    since.setDate(since.getDate() - days);
    const platforms = await this.prisma.livePlatform.findMany({
      where: { status: 'active' },
      orderBy: { sortOrder: 'asc' },
    });
    return Promise.all(

...
const PROGRESS =
  ((CURRENT_DVC - CURRENT_LEVEL_DATA.minDvc) /
    (CURRENT_LEVEL_DATA.maxDvc - CURRENT_LEVEL_DATA.minDvc)) *
  100;
      ...
      <div className="px-4 pt-4 pb-6 space-y-5">
        ...
        >
          ...
          <motion.div
            ...
            style={{
              background: `radial-gradient(circle, ${CURRENT_LEVEL_DATA.color}30 0%, transparent 70%)`,
            }}
            ...
            >
              <div>
                <p
                  ...
                  style={{ color: CURRENT_LEVEL_DATA.color }}
                >
                  L6
                </p>
                <p className="text-[10px] font-medium" style={{ color: CURRENT_LEVEL_DATA.color }}>
                  {CURRENT_LEVEL_DATA.name}
                </p>
              </div>
              {/* Pulse rings */}
              <motion.div
            ...
            >
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-caption text-text-muted">
                  距L7{CURRENT_LEVEL_DATA.name}还差{' '}
                  {formatNumber(NEXT_LEVEL_DATA.minDvc - CURRENT_DVC)} DVC
                </span>
                <span
              ...
              <div className="h-2.5 bg-bg-elevated rounded-full overflow-hidden">
                <motion.div
                  ...
                  style={{
                    background: `linear-gradient(90deg, ${CURRENT_LEVEL_DATA.color}, #F6A623)`,
                  }}
                  initial={{ width: 0 }}
          ...
          {[
            ...
            <button
              ...
              className={`flex-1 h-10 rounded-lg text-body-sm font-medium transition-all duration-200 ${
                activeTab === tab.key ? 'bg-accent text-bg-dark' : 'bg-bg-elevated text-text-muted'
              }`}
            >
              {tab.label}
            </button>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-body-sm ${isUnlocked ? 'text-text-primary' : 'text-text-muted'}`}
                          >
                            {priv.name}
                          </span>
                          {isUnlocked ? (
                            ...
                            <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-bg-elevated text-text-muted">
                              <Lock size={9} className="inline mr-0.5" />L{priv.requiredLevel}
                            </span>
                          )}
                        </div>
                        <p
                          className={`text-caption ${isUnlocked ? 'text-text-secondary' : 'text-text-muted'}`}
                        >
                          {priv.description}
                        </p>
                      </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-white/[0.08]">
                    ...
                    <th className="px-3 py-2.5 text-[10px] text-text-muted font-medium">
                      升级条件
                    </th>
                    <th className="px-3 py-2.5 text-[10px] text-text-muted font-medium">
                      DVSF权重
                      ...
                      >
                        <td className="px-3 py-2.5">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span
                              className={`text-body-sm ${isCurrent ? 'font-semibold text-text-primary' : 'text-text-secondary'}`}
                            >
                              {lvl.name}
                            </span>
                            {isCurrent && (
                              <span className="text-[8px] px-1 py-0.5 rounded bg-accent/20 text-accent font-medium">
                                当前
                              </span>
                            )}
                          </div>
                        ...
                        <td className="px-3 py-2.5 text-caption text-text-muted">{lvl.agents}个</td>
                      </motion.tr>
                    );
                  })}
                </tbody>

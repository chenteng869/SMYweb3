...
import {
  CreditCard, TrendingUp, ArrowRight, Globe, Wallet, Zap,
  ChevronRight, Landmark, Search, ArrowDown, ArrowUpLeft,
  ToggleLeft, ToggleRight, Lock, DollarSign, RefreshCw,
...
/* ─── SPV Accounts ─── */
const SPV_ACCOUNTS = [
  { id: 'spv1', name: 'TAICHU OPC LTD', currency: 'USD', balance: 18420.50, available: 16500.00, frozen: 1920.50 },
  { id: 'spv2', name: 'TAICHU TRADING LTD', currency: 'USD', balance: 8930.25, available: 8930.25, frozen: 0 },
  { id: 'spv3', name: 'TAICHU HOLDINGS LTD', currency: 'EUR', balance: 5600.00, available: 5600.00, frozen: 0 },
];
...
const CHANNEL_DATA: ChannelData[] = [
  { id: 'pay_visa', name: 'Visa/Mastercard', type: 'card', currencies: ['USD','EUR','GBP'], fee: 2.9, isActive: true, icon: 'CreditCard' },
  { id: 'pay_usdt', name: 'USDT (TRC20)', type: 'crypto', currencies: ['USDT'], fee: 0.5, isActive: true, icon: 'Coins' },
  { id: 'pay_grab', name: 'GrabPay', type: 'ewallet', currencies: ['SGD','MYR','THB'], fee: 1.8, isActive: true, icon: 'Wallet' },
  { id: 'pay_alipay', name: '支付宝', type: 'ewallet', currencies: ['CNY','HKD'], fee: 1.2, isActive: false, icon: 'Zap' },
  { id: 'pay_bank', name: '银行转账', type: 'bank', currencies: ['USD','EUR','GBP','SGD'], fee: 0, isActive: true, icon: 'Landmark' },
];
    ...
    <div className="min-h-screen pb-8">
      ...
      <div className="px-4 pt-4 pb-6 space-y-5">
        ...
        >
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {SPV_ACCOUNTS.map(acc => (
              ...
              >
                <p className={`text-body-sm font-medium ${selectedAccount.id === acc.id ? 'text-coral' : 'text-text-primary'}`}>
                  {acc.name}
                </p>
          ...
          {[
            ...
            >
              ...
              <p className={`font-display text-h3 mt-1 ${card.color}`}>
                {card.prefix}{card.value.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
              </p>
            </motion.div>
          ))}
        </motion.div>
        ...
        <motion.div
          ...
          style={{ background: 'linear-gradient(135deg, rgba(246,166,35,0.15) 0%, rgba(0,212,170,0.08) 50%, transparent 100%)' }}
        >
          <div className="flex items-center gap-2 mb-1">
            <TrendingUp size={16} className="text-coral" />
            <span className="text-caption text-text-muted">今日收入</span>
          </div>
          <p className="font-display text-[32px] text-gradient-kpi leading-tight">
            $2,480
          </p>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {exchangeRates.slice(0, 4).map((rate) => (
              <Card key={`${rate.from}-${rate.to}`} variant="default" padding="sm">
                <p className="text-[10px] text-text-muted">{rate.from}/{rate.to}</p>
                <p className="font-display text-h3 text-text-primary mt-0.5">{rate.rate.toFixed(4)}</p>
                <p className={`text-[10px] mt-0.5 ${rate.change24h >= 0 ? 'text-teal' : 'text-danger'}`}>
        ...
        >
          {[
            ...
            { label: '提现', icon: ArrowUpLeft, color: 'text-coral', bg: 'bg-coral/15', onClick: handleWithdraw },
                ...
                {withdrawStep === 1 && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
                    ...
                    <GradientButton
                      ...
                      disabled={!withdrawAmount || parseFloat(withdrawAmount) > selectedAccount.available}
                      className="w-full"
                    >
                      下一步
                    </GradientButton>
                    ...
                    {['USDT (TRC20)', '银行电汇', 'Wise'].map(method => (
                      <motion.button
                        ...
                        className={`w-full flex items-center gap-3 p-3 rounded-xl border text-left ${
                          withdrawMethod === method ? 'border-coral/40 bg-coral/10' : 'border-white/[0.08] bg-bg-card'
                        }`}
                      >
                        <CreditCard size={18} className="text-coral" />
                        <span className="text-body-sm text-text-primary flex-1">{method}</span>
                        {withdrawMethod === method && <CheckCircle size={16} className="text-coral" />}
                      </motion.button>
                    ))}
                    <div className="flex gap-2">
                      <GradientButton variant="ghost" onClick={() => setWithdrawStep(1)} className="flex-1">
                        返回
                      </GradientButton>
                      <GradientButton
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
                    <div className="bg-bg-card rounded-lg p-3 space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="text-body-sm text-text-primary">${parseFloat(withdrawAmount).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between">
                      ...
                      <div className="border-t border-white/[0.06] pt-2 flex justify-between">
                        ...
                        <span className="font-display text-h3 text-gradient-kpi">${(parseFloat(withdrawAmount) - 15).toLocaleString()}</span>
                      </div>
                    </div>
        ...
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-caption text-teal">{channelList.filter(c => c.isActive).length} 个活跃</span>
          </div>
                ...
                <Card variant="default" padding="md">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-text-muted">{ch.currencies.join(', ')} · 费率 {ch.fee}%</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-body-sm text-text-primary truncate">{txn.counterparty}</p>
                        <p className="text-[10px] text-text-muted">{txn.channelName} · {txn.description}</p>
                      </div>
                    </div>
                    <div className="text-right shrink-0 ml-2">
                      <p className={`font-display text-body-sm ${txn.type === 'incoming' ? 'text-teal' : 'text-text-primary'}`}>
                        {txn.type === 'incoming' ? '+' : '-'}{formatCurrency(txn.amount, txn.currency)}
                      </p>
                      <StatusBadge
                        status={txn.status === 'completed' ? 'success' : txn.status === 'pending' ? 'warning' : 'accent'}
                        size="sm"
                      >
                        {txn.status === 'completed' ? '完成' : txn.status === 'pending' ? '待处理' : '处理中'}
                      </StatusBadge>
                    </div>
                  </div>
                </Card>
